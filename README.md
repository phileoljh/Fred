...
- **整合 CNN 貪婪指數 (Fear & Greed Index)**：直接以原生 `requests` 與 CNN 內部 JSON API 對接（免去脆弱的 HTML 爬蟲與外部第三方庫依賴，完美共用失敗重試 Session），初始化拉取 600 天歷史，並與 VIX 指標對比呈現（預設隱藏 VIX，保持畫面聚焦）。
...
- **三版本輸出**：
  ...
  - `combined.html`：**綜合對比儀表板**，將相關聯的指標（如收入 vs 支出、VIX vs 貪婪指數）整合在同一張圖表中，方便進行連動性分析。
...
├── init_db.py          # 資料庫初始化與 CNN 貪婪指數核心爬取函式 (update_fear_greed)
├── fetch_data.py       # 負責呼叫 FRED API 並每日同步最新資料 (含貪婪指數 30 天同步)
...
### 6. CNN 貪婪指數 API 的使用限制與設計對策是什麼？
CNN 的貪婪指數（Fear & Greed Index）由於沒有公開的官方開發者 API，我們是直接與其前端背後的內部 JSON 端點進行對接。在使用上具有以下限制，專案已在設計上予以克服：
- **請求頻率與 IP 阻擋限制**：CNN 的內部 API 對非瀏覽器的 User-Agent 非常敏感，且對高頻請求有 Rate Limiting 限制。
  - *對策*：專案自訂了標準瀏覽器的 `User-Agent`，並將呼叫邏輯納入 `requests.Session`。透過共用 FRED 的失敗重試機制（`urllib3` 的 `Retry` 指數退避、5 次自動重試），可有效對抗網路波動與 API 頻率限制。
- **單次歷史點數上限**：CNN API 對單次請求回傳的歷史數據長度有限（通常不超過一年半）。
  - *對策*：本系統在初始化時，設定回推 **600 日曆天**（約 411 個實際美股交易日）作為請求歷史數據的長度。這提供了極佳的緩衝，能確保與圖表 18 個月（約 547 日曆天）的時間軸起點完全對齊，避免邊界點出現 `null` 斷點。
- **非交易日無數據**：貪婪指數僅在美股交易日更新，週末及休市期間沒有數據點。
  - *對策*：資料庫設計上，使用 SQLite `PRIMARY KEY (series_id, date)`，在圖表渲染時以所有數據的日期聯集為時間軸，即使休市日無數據點也不會導致寫入或渲染出錯。
- **內部 API 變更風險**：由於是未公開端點，CNN 有可能在無預警的情況下變更 API 路徑或 JSON 結構。
  - *對策*：我們採用原生對接方式（不依賴第三方封裝庫），如遇 API 調整，僅需直接在 `init_db.py` 中修改對應端點即可，無須等待第三方套件更新。
