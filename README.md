...
系統在抓取數據時，會同步向 FRED 官方 API 詢問該指標的「精準最終發行時間 (last_updated)」。在產生網頁的當下，只要系統判斷這筆最新的數據距離官方發布時間**還在 7 天以內**（且該指標屬於月報或季報等較低頻率），該指標的日期旁就會亮起發光的 `NEW` 徽章，讓您一目瞭然本週市場最新出爐的經濟數據。
