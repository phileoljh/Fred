...
**完全不會！**
FRED 的純數字時間序列資料非常輕薄，一組資料不到 100 Bytes。我們在設計 SQLite 資料表時強制加入了 `PRIMARY KEY (series_id, date)` B-Tree 索引樹結構。
- 就算資料累積 100 年，查詢速度依然在**千分之一秒內**。
- 目前 25 個指標的 18 個月歷史紀錄，總體檔案大小小於 1MB。
